...
import { entities } from '../album/entity/entities.js';
...
// nullish coalescing
const database = (db?.name as string | undefined) ?? Album.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Album.name.toLowerCase();
